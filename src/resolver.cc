...
// Statements
void Resolver::visit(const ExprStmt *stmt) { resolve(stmt->expression); }
...
void Resolver::visit(const PrintStmt *stmt) { resolve(stmt->expression); }
...
void Resolver::visit(const Get *expr) { resolve(&expr->callee); }
...
void Resolver::visit(const Grouping *expr) { resolve(&expr->expression); }
