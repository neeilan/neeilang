...
std::string AstPrinter::visit(const VarStmt * stmt) {
   ostringstream out;
   out << "(VARSTMT name=" << stmt->name.lexeme
...
std::string AstPrinter::visit(const IfStmt * stmt) {
  return "<BLOCK>";
  //return "if (" + visit(stmt->condition) + ") { " + visit(stmt->then_branch) + "} else {" + print(stmt->else_branch) + "}":
}
...
std::string AstPrinter::visit(const FuncStmt * stmt) {
   ...
   for (int i = 0; i < stmt->parameters.size(); i++) {
      out << stmt->parameters[i].str()
          << ": "
          << stmt->parameter_types[i].str();
    }
    out << "))";
