...
level applications (see section 3.2.3 for details). User-level applications that
like to call system calls should use the functions from the C library. The
interface between the C library and the Linux kernel is the same as for the
user-level appli- cations with the following differences:
...
template <typename T> void syscall_id(ostream &out, T arg) {
  out << "mov rax, " << arg << _endl;
}

template <typename T> void syscall_arg1(ostream &out, T arg) {
  out << "mov rdi, " << arg << _endl;
}

template <typename T> void syscall_arg2(ostream &out, T arg) {
  out << "mov rsi, " << arg << _endl;
}

template <typename T> void syscall_arg3(ostream &out, T arg) {
